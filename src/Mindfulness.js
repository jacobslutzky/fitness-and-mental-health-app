...
import { listCommunityCardss, getCommunityCards } from "./graphql/queries";
...
export default function Mindfulness() {
    ...
    // Add state for community cards
    const [communityCards, setCommunityCards] = useState([]);

    // Add state for index and routes
    ...
    // Fetch community cards and set state
    const fetchCommunityCards = async () => {
      try {
        const result = await API.graphql({
          query: listCommunityCardss,
        });

        setCommunityCards(result.data.listCommunityCards.items);
      } catch (error) {
        console.error('Error fetching community cards:', error);
      }
    };

    // Use useEffect to call fetchCommunityCards when the component mounts
    useEffect(() => {
      fetchCommunityCards();
    }, []);
